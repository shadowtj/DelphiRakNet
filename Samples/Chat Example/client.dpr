...
begin
  try
    ...
    writeln('This is a sample implementation of a text based chat client.');
    writeln('Connect to the project "Chat Example Server".');
    writeln('Difficulty: Beginner');
    writeln('');

    // Get our input
    writeln('Enter the client port to listen on');
    Readln(clientPort);
    if clientPort = EmptyStr then
      clientPort := '0';

    writeln('Enter IP to connect to');
    readln(ip);
